...
export class MainOrchestrator {
  ...
  async implementFeature(
    ...
    try {
      ...
      const repoPath = await this.repoManager.cloneRepo(repoUrl);
      spinner.succeed('Repository cloned');

      // 2. Plan
      spinner = ora('Creating implementation plan...').start();
      const demand = text ?? `Issue: ${issueUrl}`;
      const plan = await this.planner.createPlan(demand, repoPath);
      spinner.succeed(`Plan created: ${plan.title} (${plan.subtasks.length} subtasks)`);
      ...
      const changes = await this.coder.implementPlan(plan, repoPath);
      ...
      // 5. Commit + PR
      const branchName = `feature/${plan.title.toLowerCase().replace(/\s+/g, '-')}`;
      await this.repoManager.createBranch(repoPath, branchName);
      await this.repoManager.commit(repoPath, `feat: ${plan.title}`);
      const prUrl = await this.repoManager.createPullRequest({
        repoUrl,
        title: plan.title,
      ...
      return { success: true, prUrl: prUrl ?? undefined, message: 'Feature implemented successfully!' };
    } catch (err) {
